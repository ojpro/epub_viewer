...
function computeNormalizedRect(range, contents) {
  try {
    var clientRect = range.getBoundingClientRect();
    var ownerWindow = (contents && contents.window) || range.startContainer.ownerDocument.defaultView;
    var iframe = ownerWindow.frameElement;
...
function loadBook(data, cfi, initialXPath, manager, flow, spread, snap, allowScriptedContent, direction, useCustomSwipe, backgroundColor, foregroundColor, fontSize, clearSelectionOnNav, selectAnnotationRangeParam, customCss) {
  ...
  rendition.on('markClicked', function (cfiRange, data, contents) {
    try {
      var clickedRange = contents && typeof contents.range === 'function'
        ? contents.range(cfiRange)
        : null;
      var clickedRect = clickedRange ? computeNormalizedRect(clickedRange, contents) : null;
      window.flutter_inappwebview.callHandler('markClicked', cfiRange, clickedRect);
    } catch (e) {
      try {
...
function addHighlight(cfiRange, color, opacity) {
  ...
  }, "hl", { "fill": color, "fill-opacity": opacity || '0.3', "mix-blend-mode": "multiply" });
}

function addUnderLine(cfiString, color, width) {
  rendition.annotations.underline(cfiString, {}, (e) => {
  }, "ul", { "stroke": color || "black", "stroke-width": width || "3" });
}
...
function getRectFromCfi(cfiRange) {
  ...
  try {
    ...
    for (var i = 0; i < allContents.length; i++) {
      ...
      var clientRect = range.getBoundingClientRect();
      var iframe = contents.window.frameElement;
      if (!iframe) continue;

      var iframeRect = iframe.getBoundingClientRect();
      var left = iframeRect.left + clientRect.left;
      var top = iframeRect.top + clientRect.top;
      payload = {
        left: left,
        top: top,
  ...
  window.flutter_inappwebview.callHandler('cfiRect', payload);
}
